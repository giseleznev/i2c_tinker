...
// Private procedures.
void i2c_write_Bulk(uint8_t baseaddr, int32_t P1, int32_t P2, int32_t P3, uint8_t divBy4, si5351RDiv_t rdiv);
...
void si5351_Init(int32_t correction) {
    ...
    i2c_write(SI5351_REGISTER_3_OUTPUT_ENABLE_CONTROL, 0xFF);

    // Power down all output drivers
    i2c_write(SI5351_REGISTER_16_CLK0_CONTROL, 0x80);
    i2c_write(SI5351_REGISTER_17_CLK1_CONTROL, 0x80);
    i2c_write(SI5351_REGISTER_18_CLK2_CONTROL, 0x80);
    i2c_write(SI5351_REGISTER_19_CLK3_CONTROL, 0x80);
    i2c_write(SI5351_REGISTER_20_CLK4_CONTROL, 0x80);
    i2c_write(SI5351_REGISTER_21_CLK5_CONTROL, 0x80);
    i2c_write(SI5351_REGISTER_22_CLK6_CONTROL, 0x80);
    i2c_write(SI5351_REGISTER_23_CLK7_CONTROL, 0x80);

    // Set the load capacitance for the XTAL
    si5351CrystalLoad_t crystalLoad = SI5351_CRYSTAL_LOAD_10PF;
    i2c_write(SI5351_REGISTER_183_CRYSTAL_INTERNAL_LOAD_CAPACITANCE, crystalLoad);
}
...
void si5351_SetupPLL(si5351PLL_t pll, si5351PLLConfig_t* conf) {
    ...
    i2c_write_Bulk(baseaddr, P1, P2, P3, 0, 0);

    // Reset both PLLs
    i2c_write(SI5351_REGISTER_177_PLL_RESET, (1<<7) | (1<<5) );
}
...
int si5351_SetupOutput(uint8_t output, si5351PLL_t pllSource, si5351DriveStrength_t driveStrength, si5351OutputConfig_t* conf, uint8_t phaseOffset) {
    ...
    i2c_write(clkControlRegister, clkControl);
    i2c_write_Bulk(baseaddr, P1, P2, P3, divBy4, conf->rdiv);
    i2c_write(phaseOffsetRegister, (phaseOffset & 0x7F));
...
void si5351_EnableOutputs(uint8_t enabled) {
    i2c_write(SI5351_REGISTER_3_OUTPUT_ENABLE_CONTROL, ~enabled);
}

// Common code for _SetupPLL and _SetupOutput
void i2c_write_Bulk(uint8_t baseaddr, int32_t P1, int32_t P2, int32_t P3, uint8_t divBy4, si5351RDiv_t rdiv) {
    i2c_write(baseaddr,   (P3 >> 8) & 0xFF);
    i2c_write(baseaddr+1, P3 & 0xFF);
    i2c_write(baseaddr+2, ((P1 >> 16) & 0x3) | ((divBy4 & 0x3) << 2) | ((rdiv & 0x7) << 4));
    i2c_write(baseaddr+3, (P1 >> 8) & 0xFF);
    i2c_write(baseaddr+4, P1 & 0xFF);
    i2c_write(baseaddr+5, ((P3 >> 12) & 0xF0) | ((P2 >> 16) & 0xF));
    i2c_write(baseaddr+6, (P2 >> 8) & 0xFF);
    i2c_write(baseaddr+7, P2 & 0xFF);
}
